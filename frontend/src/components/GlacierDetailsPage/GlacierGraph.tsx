...
const GlacierGraph: React.FC<GlacierGraphProps> = ({
  glacierData,
  glacierId,
}) => {
  ...
  useEffect(() => {
    ...
    const data: GlacierData[] = glacierData.map((item, index) => {
      return {
        index: index + 1,
        actualMass: item.real_mass, // Теперь данные без изменений
    ...
    const tempYAxis = tempSvg.append('g').call(
      d3.axisLeft(
        d3
          .scaleLinear()
          .domain([
            d3.min(data, (d) => Math.min(d.actualMass, d.predictedMass)) ||
              -1000,
    ...
    function pointermoved(event: any) {
      ...
      tooltip
        ...
        .attr(
          'transform',
          `translate(${tooltipPosX},${y(selectedData.actualMass) - 80})`,
        );
      tooltipText.html(
    ...
    function zoom(svg: d3.Selection<SVGSVGElement, unknown, null, undefined>) {
      ...
      function zoomed(event: d3.D3ZoomEvent<SVGSVGElement, unknown>) {
        ...
        svg
          ...
          .call(d3.axisBottom(newX).tickFormat((d: any) => `Day ${d}`));

        content
          .selectAll<SVGCircleElement, GlacierData>('circle')
  ...
  return (
    <>
      <h2>Mass Change Chart for Glacier ID: {glacierId}</h2>
        <svg ref={svgRef} />
    </>
  );
};
